...
#pragma mark --------加载缓存 getHostInfo && load shared cache  **** checkSharedRegionDisable
	...
	if ( sClosureMode == ClosureMode::Off ) {
		...
		// 我们只想尝试在运行时构建一个闭包，如果它是一个iOS第三方二进制文件，或者来自共享缓存的macOS二进制文件
...
#pragma mark ---------写入缓存
		if ( (mainClosure == nullptr) && allowClosureRebuilds ) {
			...
			// 如果强制闭包，并且缓存中没有闭包，或者它是无效的，请检查缓存的闭包
...
#pragma mark --------加载动态库----loadInsertedDylib
...
#pragma mark --------链接主程序 rebase、 rebinding
...
# pragma mark --------初始化方法 initializeMainExecutable
	#if SUPPORT_OLD_CRT_INITIALIZATION
